...
%0   partial derivatives of the neural network.
%
...
J = ((1/m) * sum(sum((-y_ .* log(h)) - ((1 - y_) .* log(1 - h))))) + ((lambda/(2*m)) * (sum(sum(Theta1_.^2)) + sum(sum(Theta2_.^2))));
...
DELTA3 = A3 - y_Vec;
DELTA2 = (DELTA3 * Theta2) .* [ones(size(Z2,1),1) sigmoidGradient(Z2)]; %5000x26
...
% adding regularization term to costFunction
